...
void ListaEncadeada::remove_consecutivos(){
    node_t *node = primeiro;
    int i = 0, j = 0;
    // Caso o primeiro não seja nulo
    while(node!= NULL){
        // Caso o próximo não seja nulo
        if(node->proximo != NULL){
            // Verifica se os valores batem. Caso sejam iguais, remove o próximo e verifica novamente, sem passar para o node->proximo
            if(node->elemento == node->proximo->elemento){
                this->remover_iesimo(i+1);
            }
            else {
                // Caso o elemento não seja igual, bora pro próximo elemento, lembrando sempre de incrementar o i++ para manter o controle de qual o elemento remover
                i++;
                node = node->proximo;
            }
        }
        else{
            // Eu não sei pq escrevi isso aqui, mas to com medo de tirar e dar treta
            i++;
            node = node->proximo;
        }
    }
